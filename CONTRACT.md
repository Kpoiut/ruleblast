...
The v1 result schema remains `schemaVersion: 1` and `resolverRevision: 1`. This tree’s package version is `2.5.11`. An authorized distribution binds a published npm version to a signed source tag; registry and GitHub Release availability are external facts that this contract never infers from a source checkout.
...
v2.4.6 interprets a Codex-shaped `resolver.json` as the catalog engine. `createCodexProfile` remains the adapter oracle and is not imported by the interpreter. v2.4.7 makes that admission a list of missing resolver operations; fingerprint is not an admission input. v2.4.8 is a presentation refresh of the README demos. v2.4.9 corrects the Windows Terminal tab strip so the active tab is flush title-bar chrome, not an inset pill. v2.4.10 shortens the README demo to everyday `HEAD~1` commands and lets companion Diff From pick a recent git commit. v2.4.11 adds `--index`, a presentation-only SOURCE/CONTINUE/REJECT map of the same result for agents on large repositories. It does not add a fifth action or a product UI. It does not write a file. v2.5.0 adds the Candidate Reality Conformance Lab on `--detail` and `--receipt`: bundled `INTERPRET` / `FINGERPRINT` coverage with a sealed `oracle.json` per pack. `INTERPRET` / `ORACLE` means the live interpreter reproduced sealed `sourceDependencyPaths` and projection digests for every packed Codex fixture probe — the same JSON snapshots as `test/fixtures/codex/*.json`. Tests still compare those probes to `createCodexProfile`; the verifier does not import that adapter. `FINGERPRINT` / `ADAPTER` means the live fingerprint adapter reproduced sealed `sourceDependencyPaths` and projection digests for every packed Claude, Gemini, or Copilot fixture probe — the same JSON snapshots as `test/fixtures/claude/*.json`, `test/fixtures/gemini/*.json`, and `test/fixtures/copilot/*.json`. The sealed missing-operation list still matches live `uninterpretableReasons` and the interpreter still rejects those resolvers. That is adapter execution, not interpreter admission. Candidate fixtures carry a loadable `ManifestSnapshot` and may only record `UNKNOWN` / `UNSPECIFIED`; `LOADED` means the snapshot constructed, not a passing projection. `RECORDED` is a decoded unknown-record bound to a claim, not a passing oracle. Projection of a candidate remains `UNEXECUTED` because no interpreter-admissible resolver exists. Packed installs include `packs/candidate`. A mismatched or missing oracle fails closed. The lab does not score model quality, compliance, or downstream behavior. It does not add a fifth action, a fifth bundled reality, a harness, or a `--pack` loader. TypeScript remains the analysis authority; a native capture sidecar is not admitted while the packed 10,000-path budget holds. v2.5.1 interprets a Copilot-shaped `resolver.json` (multi-origin discover, select-all, unspecified assemble, frontmatter-glob `applyTo`, unexpanded `@` mention). `createCopilotProfile` remains the adapter oracle and is not imported by the interpreter. Claude and Gemini still use fingerprint engines because `transform` (imports / HTML comments) and Gemini `partial-unfollowed` are not data-complete. Candidate files under `packs/candidate` are not public `--reality` ids, not a fifth bundled reality, and not a `--pack` loader. Fingerprint builtins stay four. `--detail` and `--receipt` may print bundled `SEALED` / `NO_KNOWN_DRIFT` / `POSSIBLY_STALE`, candidate `NOT_ADMITTED`, and the lab matrix without changing canonical JSON. Those labels are relative to committed candidate inventory, not live vendor documentation. Companion last-result lifecycle stays `CURRENT` / `STALE`. Human overlay may restate `IN_BLAST` / `OUTSIDE_BLAST` as CONTINUE / REJECT for the next reader of the same result. REJECT is not a recommendation to discard the Git change. RuleBlast does not write intent files into the repository. A model name is never a reality: Grok the model is not Grok Build CLI; Composer is not a Cursor-editor reality; GLM is not ZCode. v2.5.2 interprets a Claude-shaped `resolver.json`: `strip-html-comments`, `at-path-import` with `claude-markdown-v1`, `json-exclude-globs`, frontmatter-glob `paths` with brace-budget matching, and `onSameBasename: partial-no-payload`. `createClaudeProfile` remains the adapter oracle and is not imported by the interpreter. Interpreter admission is the two executable families (ordered byte-budget, or select-all unspecified assemble). Gemini still fingerprints on `onSymlink` (`partial-unfollowed`) and ordered `assemble`. Copilot `applyTo` is narrow YAML (string scalar, comma-split, or string sequence). Candidate files under `packs/candidate` are not public `--reality` ids. Fingerprint builtins stay four. v2.5.3 interprets Gemini by composing select-all, ordered assemble, `partial-unfollowed` symlinks, lexer `markdown-v1`, and `json-union-names`. `createGeminiProfile` remains the adapter oracle and is not imported by the interpreter. All four bundled realities are `INTERPRET` / `ORACLE`. Pack id is the lookup key for the executing reality. Lexer `markdown-v1` is the shared markdown mechanism. Admission is capability-based, not a third interpreter family. Overlay wall and the 5 s clock are unchanged. v2.5.4 makes host shell quoting a shared `hostShellDialect()`: PowerShell on Windows, POSIX on Linux and macOS. CLI, MCP stdio, and companion explain use that dialect. Verify includes macos-latest. Canonical JSON is unchanged. v2.5.5 shares one `runGit()` spawn with `windowsHide`, defaults omitted presentation context to `hostShellDialect()`, and captures snapshot entries through `ownSnapshotEntry()`. Canonical JSON is unchanged. v2.5.6 classifies runtime surface IDs separately from model names. Candidate inventory is a generic not-admitted roster keyed by those IDs. `--reality` does not admit a fifth bundled reality. Canonical JSON is unchanged. v2.5.7 makes that classification an exact allowlist: no model-name denylist, no implied `@1` on forming candidates, no empty watch stubs, and a declared `surface` field. CLI and MCP share one `HostProcess` for platform quoting. Canonical JSON is unchanged. v2.5.9 prints implementation `ORACLE` beside `CALIBRATED` on compact lab and `--receipt`, seals bundled vendor-source observations as `ruleblast.runtime-observation.v1` (loaded paths and vendor assembly; not copied `oracle.json` projection digests; not interpreter-shaped contributions), verifies `CALIBRATED` by re-running the vendor-source observer (interpreter payload equality is `ORACLE`, not the `CALIBRATED` gate), seals `projectionDigest` as semantic stack identity and stores that seal in `oracle.json` (not a hash of a digest-bearing object), classifies overlay `DIVERGENT` from proven DIFFERENT pair events, folds replay metrics from path rows, requires vendor-dump runtime provenance (surface, revision, sealed-offline-dump, artifact digest, probe schema), canonicalizes macOS `/var` realpaths for packing/case/CLI entry, and gives MCP `pathsOnly`/`witness`. `NO_INTROSPECTION` remains the label when a dump is absent. Canonical JSON is unchanged. v2.5.10 fail-closes empty `.ruleblast-allow`, speaks MCP stdio as newline-delimited JSON-RPC per protocol `2024-11-05` while still accepting `Content-Length` as a host dialect and echoing that framing, requires string or integer request ids, does not reply to notifications, returns `-32600` for an object without `method`, returns `-32602` for an unknown tool or invalid tool arguments, returns `-32700` for invalid UTF-8 frames, requires `initialize` negotiation params as the first stdio request, advertises read-only tool annotations, rejects non-UTF-8 Git pathnames as `INVALID_PATHNAME_ENCODING` on commit and worktree capture, discovers Copilot `.claude/CLAUDE.md` and nested standard-location `.github/copilot-instructions.md` / `.claude/CLAUDE.md` / `.github/instructions/**/*.instructions.md` from the documented instruction table, keeps nested Claude `.claude/rules` inside the parent-of-`.claude` subtree, leaves Copilot instruction symlinks `UNKNOWN` without following them, admits only transform pipelines the live interpreter executes, loads production packs through that interpreter only (uninterpretable resolvers fail closed; adapters are test oracles, not a production escape hatch), interned seals compare profile/cwd/trigger/captured `sources[]`/status/composition/units/evidence not only producer-array identity, recapture mutated or extra-field producer arrays, intern units seals by units-array identity without accepting a forged digest, compile glob matchers once, keeps the packed 10,000-path efficiency gate at p95 < 1,000 ms, pins Verify Actions by full commit SHA, and runs `release:check` on a separate Release workflow. Canonical JSON is unchanged. v2.5.11 is the packed GO preflight of that chapter: local `release:check` covers `check`, packed-install smoke, production-module line bind, and 10,000-path p95 < 1,000 ms. Canonical JSON is unchanged. v2.5.8 adds sealed `calibration.json` (implementation-oracle stays `ORACLE`; pack load fails closed without the sealed file; a sealed dump matches interpreter observations or fails closed; the vendor CLI is not live authority), runtime-pair disagreement on the default two-profile path including diff newly split / converged pairs, sequential prepare capture, an executable candidate-promotion invariant, advertised-CLI MCP pins, MCP `compare` on explain, and MCP discovery for JetBrains, Visual Studio, and Neovim plus VSCodium/Roo Code on the existing companion. Canonical JSON is unchanged.

## Product claims

RuleBlast is a local, read-only, Git-native debugger for repository instruction projection. The canonical analysis engine is the authority. The CLI is the reference host. The VS Code-compatible companion in `hosts/vscode` may only render and invoke the same four actions. Cursor, Windsurf, Kiro, Antigravity, VSCodium, and Roo Code are compatible hosts of that same companion; they are not additional host classes and not modeled realities. `--mcp` is a stdio transport of the same four actions. It is not a fifth action and not a product MCP server. Given the same supported snapshot bytes, selected profiles, projection context, and resolver revision, every host produces the same canonical core result.
...
All four actions accept `--json` or deterministic text. Default explain text is the shared visual source tree (catalog badges, selected sources, changed markers, why-this-path, findings). Digests and full evidence remain in `--json`. Text accepts `--color=auto|always|never`; `NO_COLOR` disables color even when color is requested. `--witness` is opt-in: text appends why-edges derived from existing projection sources, and `--json --witness` wraps the unchanged canonical result in a `ruleblast.witness.v1` envelope. `--receipt` is opt-in: text prints a compact scoreboard box derived from the existing result plus the user-owned agent-allow state, and `--json --receipt` emits an `RBREC1` card with an `RBCTX1` identity. `--paths-only` is presentation-only: one attention path per line (diff and case: changed stacks; scan: current splits). It cannot combine with `--json`, `--witness`, `--receipt`, `--compare`, `--detail`, or `--index`. `--index` is presentation-only: a compact tab-separated map of the same result, every row, no overlay sample cap. PAIR, NEWPAIR, CONVPAIR, and INDPAIR counts are the fold of PAIRPATH, NEWPAIRPATH, CONVPAIRPATH, and INDPAIRPATH rows from one pair-event walk. SOURCE, CONTINUE, REJECT, and SPLIT remain membership rows. Agents use it on large repositories; humans can read it. It cannot combine with `--json`, `--witness`, `--receipt`, `--compare`, `--detail`, or `--paths-only`. It is not a fifth action. It does not write a file. `--detail` is presentation-only: the same canonical result as default text, with the fields default human text compresses. Default text is the summary. `--detail` cannot combine with `--json`, `--paths-only`, `--compare`, or `--receipt`. `--compare` is explain-only presentation of two selected-reality source stacks whose payload relation is proven `DIFFERENT` when such a pair exists. It cannot combine with `--json`. These flags do not change the canonical result. Agent-allow is `yes` only when the user set `RULEBLAST_AGENT_ALLOW` or created `.ruleblast-allow`; RuleBlast never writes that file and does not record live agent tool calls. `--reality github/copilot-cli@1` and `--reality google/gemini-cli@1` are opt-in documented surfaces. Both may be selected on one invocation. Default `--json` remains schema-1 two-profile result bytes with no envelope when neither flag is passed. Copilot VS Code, hosted Copilot, Gemini Code Assist, and Cursor editor semantics remain distinct unsupported surfaces. A host is not a reality: running RuleBlast inside VS Code, GitHub Copilot Chat, Cursor, Windsurf, Kiro, Antigravity, Codex IDE, Continue, Cline, Trae, Claude Desktop, or ChatGPT/Codex desktop does not model `cursor/editor@1` or `copilot/vscode@1`. `--mcp` speaks MCP stdio for those same four actions. `--help` prints usage and `--version` prints package metadata. JSON contains canonical field names and no color or presentation aliases.
...
V1 may opt in one or both additional bundled realities on one invocation by repeating `--reality`:
