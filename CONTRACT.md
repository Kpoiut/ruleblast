...
The v1 package version is `1.0.3`. Latest verified public npm remains `1.0.2` until `1.0.3` is published. Registry and GitHub Release availability are external facts that this contract never infers from a source checkout.
...
All four actions accept `--json` or deterministic text. Text accepts `--color=auto|always|never`; `NO_COLOR` disables color even when color is requested. `--help` prints usage and `--version` prints package metadata. JSON contains canonical field names and no color or presentation aliases.
