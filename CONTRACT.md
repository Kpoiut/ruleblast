...
The v1 result schema remains `schemaVersion: 1` and `resolverRevision: 1`. This tree’s package version is `2.4.6`. An authorized distribution binds a published npm version to a signed source tag; registry and GitHub Release availability are external facts that this contract never infers from a source checkout.
...
v2.4.6 interprets a Codex-shaped `resolver.json` as the catalog engine. `createCodexProfile` remains the adapter oracle and is not imported by the interpreter. Claude, Gemini, and Copilot still use fingerprint engines. Candidate files under `packs/candidate` are not public `--reality` ids, not a fifth bundled reality, and not a `--pack` loader. Fingerprint builtins stay four. `--detail` and `--receipt` may print bundled `CURRENT` / `POSSIBLY_STALE` and candidate `NOT_ADMITTED` without changing canonical JSON. Human overlay may restate `IN_BLAST` / `OUTSIDE_BLAST` as CONTINUE / REJECT for the next reader of the same result. RuleBlast does not write intent files into the repository. A model name is never a reality: Grok the model is not Grok Build CLI; Composer is not a Cursor-editor reality; GLM is not ZCode.
