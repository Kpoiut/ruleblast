...
GitHub Copilot Chat, Cursor, Windsurf, Kiro, Antigravity IDE, Codex IDE, Continue, Cline, Trae, VSCodium, and Roo Code reuse that companion. There is no per-editor fork. Pack `ruleblast-companion-2.5.11.vsix` with `npm run host:pack`. Published npm is `2.5.11`. Marketplace and Open VSX uploads are separate publisher steps.
