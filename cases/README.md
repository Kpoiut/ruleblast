...
`releaseReproductionCommand` pins the intended exact `1.0.0` package interface for reproducing the comparison after publication. It does not rewrite provenance: the producer fields remain the historical truth about the clean commit and development package that captured the receipt. Registry availability is verified independently at execution time.
