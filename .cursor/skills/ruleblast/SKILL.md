...
| Symptom | Command |
|---|---|
| What does each pinned CLI project now? | `npx --yes ruleblast@2.4.0 . --color=never` |
| Which stacks moved? | `npx --yes ruleblast@2.4.0 diff HEAD~1 --to HEAD --color=never` |
| Why this path? | `npx --yes ruleblast@2.4.0 explain <path> --from HEAD~1 --to HEAD --json` |
| Teaching receipt, no clone | `npx --yes ruleblast@2.4.0 case --color=never` |

Published CLI: `npx --yes ruleblast@2.4.0`. MCP stdio: `npx --yes ruleblast@2.4.0 --mcp`. Add `--receipt` for the compact scoreboard (per-profile inheritance, not live agent telemetry). Add `--witness` for why-edges. Add `--paths-only` for one attention path per line. Add `--compare` on explain for two selected-reality stacks. Add `--reality github/copilot-cli@1` or `--reality google/gemini-cli@1` only when that surface is requested. Repeat `--reality` for both. Default `--json` stays two-profile.
