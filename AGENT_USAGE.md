...
`[path]` only starts Git discovery. There is no `ruleblast scan` subcommand. `diff` has no `--from`. `--witness` prints why-edges. Default `--json` remains the canonical result.
