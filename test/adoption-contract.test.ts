...
describe("v2.4.0 adoption contract", () => {
  ...
  it("keeps the public onboarding on the verified case and product boundary", () => {
    ...
    expect(readme).toContain("npx --yes ruleblast@2.3.0 case");
    ...
    const yourRepo = readme.indexOf("npx --yes ruleblast@2.3.0 .");
    const missed = readme.indexOf("## What Git missed");
    const proof = readme.indexOf("PROOF.md");
    const teachingCase = readme.indexOf("npx --yes ruleblast@2.3.0 case");
  ...
  it("publishes a truthful community funnel without shipping repository-only assets", () => {
    ...
    expect(security).toMatch(/latest published `2\.3\.x`/iu);
  ...
  it("embeds What Git missed without packaging it", () => {
    ...
    const install = readme.indexOf("## Install");
    expect(heading).toBeGreaterThan(readme.indexOf("npx --yes ruleblast@2.3.0 ."));
