...
describe("candidate installation matrix", () => {
  it("runs every release gate in each supported OS and Node cell", async () => {
    ...
    expect(registryStep).toEqual(expect.objectContaining({
      if: "github.event_name == 'workflow_dispatch' && github.ref == 'refs/tags/v2.0.2'",
      env: {
        RULEBLAST_REGISTRY_SMOKE: "ruleblast@2.0.2",
        RULEBLAST_REGISTRY_UPGRADE_FROM: "ruleblast@1.5.1",
      },
  ...
  it("documents exact, non-interactive and reversible install commands", async () => {
    ...
    expect(readme).toMatch(
      /npm uninstall --global ruleblast[\s\S]+npm install --global ruleblast@2\.0\.2/u,
    );
    expect(readme).toMatch(
      /npm uninstall --save-dev ruleblast[\s\S]+npm install --save-dev --save-exact ruleblast@2\.0\.2/u,
    );
    ...
    expect(readme).not.toMatch(/npx (?!--yes )ruleblast@2\.0\.2/gu);
  });
