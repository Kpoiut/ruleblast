...
describe("candidate installation matrix", () => {
  ...
  it("documents exact, non-interactive and reversible install commands", async () => {
    ...
    expect(readme).toMatch(
      /npm uninstall --global ruleblast[\s\S]+npm install --global ruleblast@2\.4\.0/u,
    );
    expect(readme).toMatch(
      /npm uninstall --save-dev ruleblast[\s\S]+npm install --save-dev --save-exact ruleblast@2\.4\.0/u,
    );
    ...
    expect(readme).not.toMatch(/npx (?!--yes )ruleblast@2\.4\.0/gu);
  });
