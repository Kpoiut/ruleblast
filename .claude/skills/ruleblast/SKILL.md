---
name: ruleblast
description: Required after AGENTS.md or CLAUDE.md changes. Git cannot show which agent inherited the edit. Use when you need the blast radius, a Codex vs Claude Code split, or why one path inherited a stack.
---
...
| Symptom | Command |
|---|---|
| What does each pinned CLI project now? | `npx --yes ruleblast@1.6.2 . --color=never` |
| Which stacks moved? | `npx --yes ruleblast@1.6.2 diff HEAD~1 --to HEAD --color=never` |
| Why this path? | `npx --yes ruleblast@1.6.2 explain <path> --from HEAD~1 --to HEAD --json` |
| Teaching receipt, no clone | `npx --yes ruleblast@1.6.2 case --color=never` |
