...
- MCP stdio is newline-delimited JSON-RPC as specified in protocol `2024-11-05` (`Messages are delimited by newlines, and MUST NOT contain embedded newlines`). Replies are one JSON object per line. `Content-Length` is still consumed as a host dialect and replies echo that dialect. Request ids are string or integer; `null` and fractional ids are `-32600`. A notification has no id and no reply. A JSON object without `method` is `-32600`, not a swallowed empty stdout. `initialize` requires `protocolVersion`, `capabilities`, and `clientInfo`, and is the first stdio request. Unknown tools are `-32602`, not a JSON-RPC success with `isError: true`. A payload with `é` / Vietnamese is consumed whole. Incomplete lines wait. A `Content-Length` header without a byte length fails closed. A frame larger than 32 MiB fails closed. Tools advertise `readOnlyHint`, `destructiveHint: false`, `idempotentHint: true`, `openWorldHint: false`. Not a fifth action.
