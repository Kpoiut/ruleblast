...
- Local `npm run release:check` is the GO preflight: `check`, packed-install smoke, production-module line bind, and 10,000-path p95 < 1,000 ms. Advertised install pin is `ruleblast@2.5.11`. Independently verified public npm remains `ruleblast@2.5.9` until registry and GitHub Release serve the same packed bytes. Overlay wall and the 5 s clock are unchanged. Not a fifth action.
...
- MCP stdio is newline-delimited JSON-RPC as specified in protocol `2024-11-05` (`Messages are delimited by newlines, and MUST NOT contain embedded newlines`). Replies are one JSON object per line. `Content-Length` is still consumed as a host dialect and replies echo that dialect. Request ids are string or integer; `null` and fractional ids are `-32600`. A notification has no id and no reply. A JSON object without `method` is `-32600`, not a swallowed empty stdout. `initialize` requires `protocolVersion`, `capabilities`, and `clientInfo`, and is the first stdio request. Unknown tools and invalid tool arguments (exclusive flags, missing `path`) are `-32602`, not a JSON-RPC success with `isError: true`. Invalid UTF-8 in a frame is `-32700`. Allow-gate ASK remains a successful JSON-RPC result with `isError: true`. A payload with `é` / Vietnamese is consumed whole. Incomplete lines wait. A `Content-Length` header without a byte length fails closed. A frame larger than 32 MiB fails closed. Tools advertise `readOnlyHint`, `destructiveHint: false`, `idempotentHint: true`, `openWorldHint: false`. Not a fifth action.
- Git `ls-tree` / `ls-files` pathnames decode as strict UTF-8. Invalid bytes fail closed as `INVALID_PATHNAME_ENCODING` on both commit trees and tracked worktree capture. That is not a replacement-character COMPLETE path and not a mislabeled `WORKTREE_CHANGED_DURING_SNAPSHOT`. Canonical JSON path strings are unchanged for valid UTF-8. Schema 1 still names paths as UTF-8 strings, not raw Git bytes.
- Copilot CLI discover includes tracked `.claude/CLAUDE.md` from the documented instruction table (retrieved 2026-08-24). Nested `.github/copilot-instructions.md`, nested `.claude/CLAUDE.md`, and nested `.github/instructions/**/*.instructions.md` are discovered in documented standard locations (retrieved 2026-08-25). Nested files scope to the parent of `.github` / `.claude`. Modular files are not discovered in intermediate directories. A nested modular file outside the target subtree is omitted, not silently COMPLETE with a missing stack. `@path` stays `partial-unexpanded`. A tracked instruction symlink is `UNKNOWN` with `SELECTED` and zero payload bytes; the symlink target is not followed into units. Adapter `entry().kind` and interpreter `Captured.kind` match on that fixture. Pack admission admits only transform sets the live interpreter executes (empty; Claude comment+import+exclude; Gemini import+union), not every known kind in combination. Vendor CLI is not live authority.
- Claude nested `.claude/rules` files load on demand in the directory that contains `.claude` (retrieved 2026-08-25). A `packages/api/.claude/rules` file does not apply to `packages/ui`. Path-scoped rules still require `paths` frontmatter to match inside that subtree. User/managed settings and `settings.local.json` merge remain outside repository-only analysis.
- Production pack load is the data interpreter only. An uninterpretable `resolver.json` fails closed as `INVALID_PACK`; handwritten Claude/Gemini/Copilot adapters stay test oracles and are not imported by `profileFromCompiledPack`. External `--profile-pack` remains unadmitted. That finishes the v1 Reality Pack prerequisite (bundled profiles on the IR, no executable escape hatch) without opening third-party pack loading.
- Packed 10,000-path `analyzeCurrent` keeps the same projection digest recipe and the same capture contract. Directory-scoped interpreter material plus interned capture/seals keep the efficiency gate at p95 < 1,000 ms. Interned identity seals compare profile, cwd, trigger, captured `sources[]`, status, composition, units, and evidence. Capture intern recaptures when a producer array mutates or grows extra fields; extra source fields still fail closed. Units seals intern by units-array identity without accepting a forged digest. Glob matchers compile once per pattern. Captured source/unit/evidence arrays are frozen so aliasing cannot rewrite a later path. Ceiling remains 2,000 ms.
- Verify pins `actions/checkout` and `actions/setup-node` to full commit SHAs. A separate Release workflow (`workflow_dispatch` and `v*` tags, one Ubuntu/Node 20 cell) runs `npm run release:check`. Matrix Verify is still build + check. Published CLI remains `ruleblast@2.5.9`.
- Case capture materializes producer blobs through one `git cat-file --batch` instead of one spawn per path, reads dependency files concurrently, and does not re-hash the installed closure during the compile-time verify. After analysis it still re-seals artifact and dependency digests. The 60 s canonical-receipt gate is unchanged.
- Package-smoke temp directories and their parent contain no spaces, so Windows `npm exec --offline --package <tarball>` can address the path instead of hanging until the 45 s cell clock. `EXTRACTION_REVIEWS.json` is `{path, sha256, reason, followUp}`. No production module is over 400 lines, so `reviews` is empty. Check includes that bind. `npm run benchmark` counts `NO_INTROSPECTION` rows instead of a missing identifier. Gemini pack pin is not moved without a loader-claim delta. Published CLI remains `ruleblast@2.5.9`. Overlay wall and the 5 s clock are unchanged.
