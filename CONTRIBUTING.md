...
This tree is RuleBlast `v2.1.1`. Latest independently verified public npm release is `v2.1.0` until an authorized `2.1.1` publication. Fastest contribution: [surprising result](https://github.com/Kpoiut/ruleblast/issues/new?template=surprising-result.yml) — command, observed text, one sentence. No canonical JSON. Promoted Blast Cases stay the evidence gate.
