...
`.ruleblast-allow` empty/garbage/unreadable is `ask`. MCP stdio is newline-delimited JSON-RPC per the pinned protocol id; `Content-Length` is a host dialect and replies echo inbound framing. Request ids are string or integer; notifications have no reply; missing `method` is `-32600`; unknown tools and invalid tool arguments are `-32602`; invalid UTF-8 frames are `-32700`; `initialize` is first and requires negotiation params. Git pathnames that are not UTF-8 fail closed as `INVALID_PATHNAME_ENCODING` on commit and worktree capture, not as a changing worktree. Schema 1 still names paths as UTF-8 strings. Copilot discover includes `.claude/CLAUDE.md` (24 August 2026) and nested standard-location `.github/copilot-instructions.md`, `.claude/CLAUDE.md`, and `.github/instructions/**/*.instructions.md` (25 August 2026). A Copilot instruction symlink stays `UNKNOWN` and is not followed. Nested Claude `.claude/rules` stay inside the parent-of-`.claude` subtree. Pack admission admits only transform sets the live interpreter executes. Interned seals compare profile, cwd, trigger, captured `sources[]`, status, composition, units, and evidence; capture intern recaptures mutated or extra-field producer arrays; units seals intern by units-array identity without accepting a forged digest; glob matchers compile once; captured arrays are frozen. Directory-scoped interpreter material keeps the packed 10k efficiency gate at p95 < 1,000 ms. Verify Actions are pinned by full commit SHA. `release:check` runs on a separate Release workflow, not every matrix cell. Package-smoke temp roots and parent have no spaces so Windows `npm exec` can address the tarball. Extraction reviews bind over-limit modules by path and digest; none currently exceed 400 lines. Benchmark counts `NO_INTROSPECTION`. Gemini pin is not restated as a new loader. Overlay wall still four analysis pairs. Production `profileFromCompiledPack` is the data interpreter only; an uninterpretable resolver is `INVALID_PACK`. Handwritten adapters remain test oracles. External `--profile-pack` stays unadmitted. Not a kernel. Not a fifth action. Not a fifth bundled reality.

## **SHIPPED TO MAIN** — `v2.5.11`: Packed GO preflight

This tree is `2.5.11`. Latest independently verified public npm distribution remains `2.5.9`. Companion version is `2.5.11`. Pack `ruleblast-companion-2.5.11.vsix`. Do not overwrite Marketplace `2.2.0` or `2.2.1`.

Local `npm run release:check` is the GO preflight for this chapter: `check`, packed-install smoke, production-module line bind, and 10,000-path p95 < 1,000 ms. Canonical JSON, overlay wall, and the 5 s clock are unchanged. Not a kernel. Not a fifth action. Not a fifth bundled reality.
