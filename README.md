...
<p align="center">
  <img src="assets/ruleblast-hero.png?v=2.5.1s" alt="RuleBlast — See the second diff. Local, read-only, evidence-first" width="100%">
</p>
...
<p align="center">
  ...
  <a href="https://github.com/Kpoiut/ruleblast/releases/tag/v2.5.1"><img src="https://img.shields.io/github/package-json/v/Kpoiut/ruleblast" alt="this tree 2.5.1"></a>
...
<div align="center">
  <img src="assets/ruleblast-causal-proof.gif?v=2.5.1s" alt="Terminal demo: git sees 3 files and 6 deletions; ruleblast diff shows Codex 206 paths and Claude Code 0; explain names nested AGENTS.md" width="100%">
</div>

```bash
cd <your-git-repository>
npx --yes ruleblast@2.5.1 .
npx --yes ruleblast@2.5.1 diff HEAD~1
```

<p align="center"><sub>This tree is 2.5.1. Published CLI is ruleblast@2.5.1. Local · read-only · deterministic · no network or model call</sub></p>
...
<div align="center">
  <img src="assets/ruleblast-visual-benchmark.png?v=2.5.1s" alt="RuleBlast 2.4.6 scoreboard: Git saw 2 instruction lines; Codex 206, Claude Code 0; nested AGENTS.md; ALIGNED CONTINUE 2" width="100%">
</div>
...
Published CLI is `ruleblast@2.5.1`. Node.js 20+. `npx` downloads and runs the pinned package.
...
```bash
node --version
npm view ruleblast@2.5.1 version
npx --yes ruleblast@2.5.1 --help
npm install --global ruleblast@2.5.1
ruleblast --version
ruleblast --help
ruleblast
npm install --save-dev --save-exact ruleblast@2.5.1
npx ruleblast --version
npx ruleblast --help
npx ruleblast
npx --yes ruleblast@2.5.1 explain src/args.ts --from HEAD~1
npx --yes ruleblast@2.5.1 case
```
...
npm cache verify
npx --yes ruleblast@2.5.1 --help
git clone --branch v2.5.1 --depth 1 https://github.com/Kpoiut/ruleblast.git
cd ruleblast
...
```bash
npx --yes ruleblast@2.5.1 case
npx --yes ruleblast@2.5.1 case --json
npx --yes ruleblast@2.5.1 case --explain .github/ISSUE_TEMPLATE/missing-blast.yml
```
...
```bash
npx --yes ruleblast@2.5.1 .
npx --yes ruleblast@2.5.1 packages/api/internal
npx --yes ruleblast@2.5.1 diff HEAD~1
npx --yes ruleblast@2.5.1 explain src/args.ts --from HEAD~1
npx --yes ruleblast@2.5.1 diff HEAD~1 --json
```
...
npx --yes ruleblast@2.5.1 . --receipt
```
...
- uses: actions/checkout@v4
  with:
    fetch-depth: 0
- uses: Kpoiut/ruleblast@v2.5.1
```

Root Action `Kpoiut/ruleblast@v2.5.1` posts a `--receipt` for `base.sha → head.sha`. It runs published `ruleblast@2.5.1`. Pin a commit after you trust the workflow.
...
This tree is `2.5.1`. Published CLI is `ruleblast@2.5.1`. The product is overlay, work map, change alignment, and intent — which files now receive different AI instructions, and whether later work should CONTINUE or REJECT that inheritance. REJECT is not a recommendation to discard the Git change. Glance, `--paths-only`, `--index`, `explain --compare`, and a `PROOF` line disclose that same result. Offline `--detail` / `--receipt` can say a pinned evidence revision is SEALED, NO_KNOWN_DRIFT, or POSSIBLY_STALE, and print the Candidate Reality Conformance Lab, without a network fetch. Lab `ORACLE` is a sealed interpreter match on every packed Codex and Copilot fixture probe. Lab `ADAPTER` is a sealed fingerprint-adapter match on every packed Claude and Gemini fixture probe. `RECORDED` is not a passing oracle. Candidate `LOADED` is a snapshot that constructed, not a passing projection. Grok Build CLI can sit in the lab as `NOT_ADMITTED` without becoming a fifth `--reality`. It is not a model-quality score.
