...
<p align="center">
  <img src="assets/ruleblast-hero.png?v=2.4.1" alt="RuleBlast — See the second diff. Local, read-only, evidence-first" width="100%">
</p>
...
<p align="center">
  ...
  <a href="https://github.com/Kpoiut/ruleblast/releases/tag/v2.4.1"><img src="https://img.shields.io/github/package-json/v/Kpoiut/ruleblast" alt="this tree 2.4.1"></a>
...
<div align="center">
  <img src="assets/ruleblast-causal-proof.gif?v=2.4.1" alt="Terminal demo: git sees 3 files and 6 deletions; ruleblast diff shows Codex 206 paths and Claude Code 0; explain names nested AGENTS.md" width="100%">
</div>

```bash
cd <your-git-repository>
npx --yes ruleblast@2.4.1 .
npx --yes ruleblast@2.4.1 diff HEAD~1
```

<p align="center"><sub>This tree is 2.4.1. Published CLI is ruleblast@2.4.1. Local · read-only · deterministic · no network or model call</sub></p>
...
<div align="center">
  <img src="assets/ruleblast-visual-benchmark.png?v=2.4.1" alt="Square RuleBlast 2.4.1 scoreboard: Git saw 2 instruction lines; Codex 206, Claude Code 0; why-this-path, CLI and IDE surfaces, user allow gate" width="100%">
</div>
...
Published CLI is `ruleblast@2.4.1`. Node.js 20+. `npx` downloads and runs the pinned package.
...
```bash
node --version
npm view ruleblast@2.4.1 version
npx --yes ruleblast@2.4.1 --help
npm install --global ruleblast@2.4.1
ruleblast --version
ruleblast --help
ruleblast
...
npx ruleblast --help
npx ruleblast
npx --yes ruleblast@2.4.1 explain src/args.ts --from HEAD~1
npx --yes ruleblast@2.4.1 case
```
...
```bash
npx --yes ruleblast@2.4.1 case
npx --yes ruleblast@2.4.1 case --json
npx --yes ruleblast@2.4.1 case --explain .github/ISSUE_TEMPLATE/missing-blast.yml
```
...
This tree discloses that same result in layers: one-line glance, `--paths-only`, `explain --compare`, then a `PROOF` line. Published `2.4.1` includes those presentation flags.
...
```bash
npx --yes ruleblast@2.4.1 .
npx --yes ruleblast@2.4.1 packages/api/internal
npx --yes ruleblast@2.4.1 diff HEAD~1
npx --yes ruleblast@2.4.1 explain src/args.ts --from HEAD~1
npx --yes ruleblast@2.4.1 diff HEAD~1 --json
```
...
npx --yes ruleblast@2.4.1 . --receipt
```
...
- uses: actions/checkout@v4
  with:
    fetch-depth: 0
- uses: Kpoiut/ruleblast@v2.4.1
```

Root Action `Kpoiut/ruleblast@v2.4.1` posts a `--receipt` for `base.sha → head.sha`. It runs published `ruleblast@2.4.1`. Pin a commit after you trust the workflow.
...
This tree is `2.4.1`. Published CLI is `ruleblast@2.4.1`. The product is overlay, work map, and change alignment — which files now receive different AI instructions. Glance, `--paths-only`, `explain --compare`, and a `PROOF` line disclose that same result.
