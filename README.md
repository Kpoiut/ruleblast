...
<p align="center"><sub>This tree is 2.5.11. Published CLI is ruleblast@2.5.11. Local · read-only · deterministic · no network or model call</sub></p>
...
Published CLI is `ruleblast@2.5.11`. Node.js 20+. `npx` downloads and runs the pinned package.
...
npx --yes ruleblast@2.5.11 case --explain .github/ISSUE_TEMPLATE/missing-blast.yml
```
...
```bash
npx --yes ruleblast@2.5.11 .
npx --yes ruleblast@2.5.11 packages/api/internal
npx --yes ruleblast@2.5.11 diff HEAD~1
npx --yes ruleblast@2.5.11 explain src/args.ts --from HEAD~1
npx --yes ruleblast@2.5.11 diff HEAD~1 --json
```
...
npx --yes ruleblast@2.5.11 . --receipt
```
...
- uses: actions/checkout@v4
  with:
    fetch-depth: 0
- uses: Kpoiut/ruleblast@v2.5.11
```

Root Action `Kpoiut/ruleblast@v2.5.11` posts a `--receipt` for `base.sha → head.sha`. It runs published `ruleblast@2.5.11`. Pin a commit after you trust the workflow.
