...
import type { CapturedClaudeFile } from "./claude-imports.js";
...
function parseFrontmatter(value: string): Frontmatter {
  ...
  if (normalized.startsWith("---\r")) return { kind: "malformed" };
  if (!normalized.startsWith("---\n")) {
    return { kind: "ok", body: value, paths: null };
  }
  ...
  let forbidden = document.errors.length > 0 || document.warnings.length > 0;
  ...
  if (forbidden || root === null || !isMap(root) ||
      root.items.some((pair) => !isScalar(pair.key) || pair.key.value !== "paths")) {
    return { kind: "malformed" };
  }
  let paths: string[] | null = null;
  if (root.items.length > 0) {
    const node = root.items[0]?.value;
    if (node === null || node === undefined || !isSeq(node) ||
  ...
  return {
    kind: "ok",
    body: normalized.slice(after + (normalized[after] === "\n" ? 1 : 0)),
    paths,
  };
}
